...
export default function RolesPage() {
  ...
  const handleDelete = async (role: RoleDTO) => {
    if (!confirm(`Bạn có chắc muốn xóa role "${role.name}" không?`)) {
      setActiveMenu(null);
      return;
    }
  ...
  return (
    <div className="space-y-12 max-w-[1400px] mx-auto pb-40 animate-in fade-in duration-1000">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8">
        {roles.map((role) => {
          ...
          return (
            <div
              key={role.id}
              className="group glass-dark border border-slate-200 dark:border-white/5 p-8 rounded-[42px] hover:border-indigo-300 dark:hover:border-indigo-500/30 transition-all duration-700 relative flex flex-col h-full shadow-lg dark:shadow-2xl hover:shadow-xl dark:hover:shadow-indigo-500/10 hover:-translate-y-2 overflow-hidden"
            >
                ...
                <div className="relative">
                  <button
                    ...
                    className={`p-3 rounded-2xl transition-all ${
                      isMenuOpen ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-400 dark:text-white/30 hover:text-slate-900 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-white/5'
                    }`}
                  >
                  ...
                  {isMenuOpen && (
                    <div className="absolute right-0 top-full mt-3 w-48 bg-white dark:bg-slate-800 glass-dark border border-slate-200 dark:border-white/10 rounded-[24px] shadow-xl dark:shadow-3xl p-2 z-[60] animate-in zoom-in-95 duration-200">
                      <button
                        onClick={() => handleEdit(role)}
                        className="w-full flex items-center gap-3 p-3.5 rounded-2xl hover:bg-indigo-50 dark:hover:bg-white/5 text-sky-600 dark:text-sky-400 transition-all text-[10px] font-black uppercase tracking-widest active:scale-95"
                      >
                      ...
                      {!isSystemRole && (
                        <button
                          ...
                          className="w-full flex items-center gap-3 p-3.5 rounded-2xl hover:bg-rose-50 dark:hover:bg-rose-500/10 text-rose-600 dark:text-rose-500 transition-all text-[10px] font-black uppercase tracking-widest active:scale-95"
                        >
              ...
              <div className="flex-1 space-y-6 mb-10 relative z-10">
                ...
                <div className="flex flex-wrap gap-2.5">
                  {role.permissions?.slice(0, 8).map((permissionCode) => (
                    <div
                      key={permissionCode}
                      className="px-3.5 py-1.5 rounded-xl bg-slate-100 dark:bg-white/5 border border-slate-200 dark:border-white/5 text-[9px] font-black text-slate-600 dark:text-white/60 uppercase tracking-widest flex items-center gap-2 group/perm hover:bg-indigo-50 dark:hover:bg-indigo-600/10 hover:border-indigo-200 dark:hover:border-indigo-500/20 hover:text-indigo-600 dark:hover:text-indigo-400 transition-all"
                    >
                      <CheckCircle2 className="w-3 h-3 opacity-50 group-hover/perm:opacity-100 transition-opacity" />
                      {permissionCode}
                    </div>
                  ))}
              ...
              {/* Footer */}
              <div className="pt-8 border-t border-slate-200 dark:border-white/5 flex items-center justify-between mt-auto relative z-10">
      ...
      {activeMenu && <div className="fixed inset-0 z-50 mt-0" onClick={() => setActiveMenu(null)} />}
    </div>
  );
}
