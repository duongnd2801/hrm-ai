...
export default function CompanyConfigForm({ readOnly = false }: CompanyConfigFormProps) {
  ...
  const inputClass = "w-full bg-slate-900/5 dark:bg-white/5 border border-black/5 dark:border-white/10 rounded-2xl py-3 px-6 text-sm text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-indigo-500/50 transition-all shadow-inner disabled:opacity-60 disabled:cursor-not-allowed";
      ...
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          <div>
            ...
            <input disabled={readOnly} type="time" name="workStartTime" value={config.workStartTime} onChange={handleChange} className={inputClass} />
          </div>
          <div>
            <label className={labelClass}>Giờ kết thúc</label>
            <input disabled={readOnly} type="time" name="workEndTime" value={config.workEndTime} onChange={handleChange} className={inputClass} />
          </div>
          <div>
            <label className={labelClass}>Bắt đầu nghỉ trưa</label>
            <input disabled={readOnly} type="time" name="lunchBreakStart" value={config.lunchBreakStart} onChange={handleChange} className={inputClass} />
          </div>
          <div>
            <label className={labelClass}>Kết thúc nghỉ trưa</label>
            <input disabled={readOnly} type="time" name="lunchBreakEnd" value={config.lunchBreakEnd} onChange={handleChange} className={inputClass} />
          </div>
        </div>
      </div>
      ...
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          <div>
            ...
            <input disabled={readOnly} type="number" step="0.5" name="standardHours" value={config.standardHours} onChange={handleChange} className={inputClass} />
          </div>
          <div>
            <label className={labelClass}>Số ngày công chuẩn/tháng</label>
            <input disabled={readOnly} type="number" name="standardDaysPerMonth" value={config.standardDaysPerMonth} onChange={handleChange} className={inputClass} />
          </div>
          <div>
            <label className={labelClass}>Ngày chốt công</label>
            <input disabled={readOnly} type="number" name="cutoffDay" value={config.cutoffDay} onChange={handleChange} className={inputClass} />
          </div>
        </div>
      </div>
      ...
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          <div>
            ...
            <input disabled={readOnly} type="number" step="0.1" name="otRateWeekday" value={config.otRateWeekday} onChange={handleChange} className={inputClass} />
          </div>
          <div>
            <label className={labelClass}>OT Cuối tuần</label>
            <input disabled={readOnly} type="number" step="0.1" name="otRateWeekend" value={config.otRateWeekend} onChange={handleChange} className={inputClass} />
          </div>
          <div>
            <label className={labelClass}>OT Ngày lễ</label>
            <input disabled={readOnly} type="number" step="0.1" name="otRateHoliday" value={config.otRateHoliday} onChange={handleChange} className={inputClass} />
          </div>
          <div>
            <label className={labelClass}>Nghỉ lễ bù</label>
            <input disabled={readOnly} type="number" step="0.1" name="otRateHolidayComp" value={config.otRateHolidayComp} onChange={handleChange} className={inputClass} />
          </div>
          <div className="lg:mt-6">
            <label className={labelClass}>Nửa ngày Sáng</label>
            <input disabled={readOnly} type="number" step="0.1" name="halfDayMorningRate" value={config.halfDayMorningRate} onChange={handleChange} className={inputClass} />
          </div>
          <div className="lg:mt-6">
            <label className={labelClass}>Nửa ngày Chiều</label>
            <input disabled={readOnly} type="number" step="0.1" name="halfDayAfternoonRate" value={config.halfDayAfternoonRate} onChange={handleChange} className={inputClass} />
          </div>
        </div>
      </div>
