...
export default function ProjectsPage() {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case 'ACTIVE': return <span className="bg-emerald-500 text-white px-2.5 py-1 rounded-lg text-[9px] font-black uppercase tracking-widest whitespace-nowrap">Hoạt động</span>;
      case 'OVERDUE': return <span className="bg-rose-500 text-white px-2.5 py-1 rounded-lg text-[9px] font-black uppercase tracking-widest whitespace-nowrap">Quá hạn</span>;
      case 'COMPLETED': return <span className="bg-indigo-500 text-white px-2.5 py-1 rounded-lg text-[9px] font-black uppercase tracking-widest whitespace-nowrap">Hoàn thành</span>;
      default: return <span className="bg-slate-400 text-white px-2.5 py-1 rounded-lg text-[9px] font-black uppercase tracking-widest whitespace-nowrap">{status}</span>;
    }
  };
  ...
  return (
    <div className="space-y-12 pb-20">
      ...
      <div className="bg-white/60 dark:bg-white/5 backdrop-blur-3xl rounded-[48px] p-8 md:p-10 border border-black/5 dark:border-white/10 shadow-3xl overflow-hidden relative group">
                    ...
                    <tr key={p.id} className="group hover:bg-slate-50 dark:hover:bg-white/5 transition-all duration-300">
                      <td className="px-6 py-2.5 border-b border-black/5 dark:border-white/5">
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            <h3 className="font-black text-slate-900 dark:text-white text-[12px] uppercase tracking-wider whitespace-normal break-words leading-tight">{p.name}</h3>
                            <p className="text-[9px] font-bold text-indigo-400 uppercase tracking-widest mt-0.5">{p.code}</p>
                          </div>
                        </div>
                      </td>
                      <td className="px-6 py-2.5 border-b border-black/5 dark:border-white/5 max-w-[250px] whitespace-normal break-words italic text-slate-600 dark:text-white/40 font-medium text-[10px] leading-relaxed">
                        {p.description || '...'}
                      </td>
        ...
        {/* Pagination at bottom - Stable */}
        {filteredProjects.length > pageSize && (
            <div className="flex items-center justify-between border-t border-black/5 dark:border-white/10 mt-8 pt-6 relative z-10 animate-in fade-in slide-in-from-bottom-2 duration-500">
