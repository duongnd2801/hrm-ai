    ...
    <div className="space-y-8 pb-20 px-2 lg:px-6">
      ...
      {viewMode === 'calendar' ? (
        ...
        <div className="space-y-10 animate-in slide-in-from-bottom-5 duration-700">
            {/* Team Dashboard Cards - 'Anh 2' Style */}
            {!selectedUser && (
            ...
            {selectedUser ? (
               <div className="space-y-8 animate-in zoom-in-95 duration-500">
                ...
                <div className="bg-white/80 dark:bg-white/5 backdrop-blur-xl rounded-[48px] border border-black/5 dark:border-white/5 overflow-hidden shadow-2xl">
                  <div className="p-10 border-b border-black/5 dark:border-white/5 flex flex-col md:flex-row justify-between items-center gap-6">
                    <div>
                      <h2 className="text-2xl font-black text-slate-900 dark:text-white uppercase tracking-tighter italic flex items-center gap-4">
                        <Calendar className="w-8 h-8 text-slate-300 dark:text-white/20" />
                        Quản lý thành viên
                      </h2>
                      <p className="text-[10px] font-black text-slate-400 dark:text-white/20 uppercase tracking-[0.4em] mt-3 ml-12">Dữ liệu chi tiết tháng {month}/{year}</p>
                    </div>
                    <div className="flex items-center gap-4 bg-black/5 dark:bg-white/5 p-2 rounded-3xl border border-black/5 dark:border-white/5">
                       <button onClick={() => setMonth(m => m > 1 ? m - 1 : 12)} className="p-4 hover:bg-indigo-600 hover:text-white rounded-2xl transition-all active:scale-90"><ChevronLeft size={20}/></button>
                       ...
                       <button onClick={() => setMonth(m => m < 12 ? m + 1 : 1)} className="p-4 hover:bg-indigo-600 hover:text-white rounded-2xl transition-all active:scale-90"><ChevronRight size={20}/></button>
                    </div>
                  </div>
                  <div className="overflow-x-auto">
                    <table className="w-full text-left">
                      <thead className="text-[11px] font-black uppercase text-slate-400 dark:text-white/20 tracking-widest italic border-b border-black/5 dark:border-white/5 bg-white/80 dark:bg-white/5">
                        <tr>
                      ...
                      <tbody className="divide-y divide-black/5 dark:divide-white/5">
                        {teamSummary.length === 0 ? (
                          ...
                          teamSummary.map((row) => (
                            <tr key={row.employeeId} className="hover:bg-indigo-600/5 dark:hover:bg-indigo-600/10 group/row transition-all duration-300">
                              <td className="px-12 py-7">
                                 <div className="flex items-center gap-6">
                                    <Avatar name={row.employeeName} size="lg" />
                                    <div className="flex flex-col">
                                       <span className="text-[16px] font-black text-slate-900 dark:text-white group-hover/row:text-indigo-600 dark:group-hover/row:text-indigo-400 transition-colors uppercase tracking-tight">{row.employeeName}</span>
                                       <span className="text-[10px] font-bold text-slate-400 dark:text-white/20 uppercase tracking-widest mt-1">{row.departmentName}</span>
                                    </div>
                                 </div>
                              </td>
