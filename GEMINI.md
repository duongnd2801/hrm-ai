## Canonical Project Snapshot (2026-04-17)
...
- Project status: `Refinement & UX Patch Completed`
- Current focus:
  - Error Boundary (`error.tsx`) & Loading Skeleton (`loading.tsx`) cho Next.js
  - Bổ sung Unit Test cho `AttendanceService`
  - Refactor CORS config để load từ application.yml
...
| Phase Hardening | Bảo mật chuyên sâu: HttpOnly Cookies, CSRF, Security Headers, HS256, @Valid | ✅ | Đã hoàn thành 03/04/2026 |
...
| Phase EMP Access | Sữt quyền EMPLOYEE: Chỉ xem & cập nhật thông tin bản thân (EMP_VIEW_ALL permission) | ✅ | Đã hoàn thành 17/04/2026 |
| Phase Code Quality | GlobalExceptionHandler + DB Index Attendance + Audit Coverage + README JWT fix | ✅ | Đã hoàn thành 17/04/2026 |
| Phase Refinement | UX (Error Boundary, Loading), Test AttendanceService, CORS Config | ✅ | Đã hoàn thành 17/04/2026 |
...
- **Audit Logging**: Lưu trữ có dấu vết các hành động nhạy cảm như "Tính lương", "Duyệt đơn tha tội", "Tính lại chấm công".
- **Rate Limiting chống Brute Force**: Tự động block IP/Account nếu đăng nhập sai 10 lần liên tiếp (10 phút).
...
| Cache/RateLimit| **Redis 7 (Docker)** — chặn Brute Force Login 10 lần/10 phút |
| Auth | Spring Security + JWT (**HS256 / HMAC-SHA256**) + Dynamic RBAC |
| Audit Log | `AuditService` ghi nhận lịch sử các action vào DB (`audit_logs`) |
...
| Import/Export BE | Apache POI (Batch chunk processing: 100 rows/page) |
| DB Migration | Flyway |
| Build BE | Maven |
...
```sql
-- Hệ thống RBAC nâng cấp (V17-V23)
CREATE TABLE roles (
...
CREATE TABLE users (
  ...
  role_id    UUID REFERENCES roles(id), -- Quan hệ FK thay cho Enum
...
CREATE TABLE employees (
  ...
  bio             TEXT,
  gender          gender_type,
  citizen_id      VARCHAR(20),                   -- CCCD (V24)
  join_date       DATE,                          -- Ngày vào làm (V24)
  birth_date      DATE,
...
```
totalHours = (checkOut - checkIn) - lunchBreakDuration## Canonical Project Snapshot (2026-04-17)
